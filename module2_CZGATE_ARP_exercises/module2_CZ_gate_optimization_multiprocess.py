...
if __name__ == '__main__':
    ...
    task_name = 'CZ_gate_ARP_RydbergB_T1.08us'  # Modify this for different runs
    
    # ========== Optimization Parameters ==========
    # Rydberg blockade strengths to optimize [MHz]
    # For full scan, use:
    scale_B_list = [1625, 1750, 1875, 2000, 2125, 2250, 2375, 2500, 2625, 2750, 2875, 3000]
    
    # Parameter bounds
    bounds = {
        'T_gate': (1.08, 1.08), # 0.27 / 0.54 / 1.08 μs options
        'tau_ratio': (0.175, 0.175), # Fixed tau_ratio for ARP ( why? )
        'amp_Omega_r': (6*2*np.pi, 7*2*np.pi), # 15 / 10 / 5
        'amp_Delta_r': (8*2*np.pi, 9*2*np.pi)  # 50 / 30 / 25
    }

    initial_params = {
        'T_gate': 1.08,
        'tau_ratio': 0.175,
        'amp_Omega_r': 6.5 * 2 * np.pi,
        'amp_Delta_r': 8.2 * 2 * np.pi
    }
    initial_params_list = [initial_params]
